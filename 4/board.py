class Board():
    ...
    def get_potential_rows(self):
        ...
        row4 = slice(15, 20)
        row5 = slice(20, 25)
        return [self.board_input[row1], self.board_input[row2], self.board_input[row3], self.board_input[row4], self.board_input[row5]]
    ...
    def get_potential_columns(self):
        ...
        return [self.board_input[column1], self.board_input[column2], self.board_input[column3],self.board_input[column4],self.board_input[column5]]

    def get_potential_winners(self):
        return self.get_potential_rows() + self.get_potential_columns()

    def mark(self, number_called):
        self.board_input = ['x' if number == number_called else number for number in self.board_input]
